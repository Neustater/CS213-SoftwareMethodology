...
public class Date implements Comparable<Date>{
    ...
    public Date(String date) {
        ...
        final String INVALID_PARAM = "-1";  //sets value to -1 if part of date is missing

        StringTokenizer dateToken = new StringTokenizer(date,"/");

        monthSt = dateToken.hasMoreTokens() ? dateToken.nextToken() : INVALID_PARAM;
        daySt = dateToken.hasMoreTokens() ? dateToken.nextToken() : INVALID_PARAM;
        yearSt = dateToken.hasMoreTokens() ? dateToken.nextToken() : INVALID_PARAM;

        if(dateToken.hasMoreTokens()){      //if more fields exists (ie: mm/dd/yyyy/xx) date values set invalid
            month = Integer.parseInt(INVALID_PARAM);
            day = Integer.parseInt(INVALID_PARAM);
            year = Integer.parseInt(INVALID_PARAM);
        }
        else {
    ...
    public int daysInMonth(int year, int month){
        ...
        {
            lastDay = Month.THIRTY_DAYS;
        }
        else if (month == Month.FEBRUARY)  //Checks if month is a 29 day month and sets lastDay
        {
            if (isLeapYear) {
                lastDay = Month.TWENTY_NINE_DAYS;
            } else {
                lastDay = Month.TWENTY_EIGHT_DAYS;
            }
        }
        else{
            lastDay = INVALID_DATE;
        }
        return lastDay;
    }
